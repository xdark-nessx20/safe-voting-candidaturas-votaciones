...
El Gestor Electoral inscribe un candidato en una votación creando una candidatura, definiendo así la oferta electoral
...
1. **Scenario**: Inscripción exitosa de candidatura
    ...
    - **When** el Gestor Electoral inscribe la candidatura
...
2. **Scenario**: Inscribir candidatura en votación no `ACTIVA`
    ...
    - **When** el Gestor Electoral intenta inscribir una candidatura
...
3. **Scenario**: Inscribir candidato inactivo o suspendido
    ...
    - **When** el Gestor Electoral intenta inscribir su candidatura
...
El Gestor Electoral desasigna un candidato de una votación cancelando su candidatura mientras la votación esté en
estado `ACTIVA`.
...
1. **Scenario**: Cancelación exitosa de candidatura
    ...
    - **When** el Gestor Electoral la cancela
...
3. **Scenario**: Cancelar candidatura ya cancelada
    ...
    - **When** el Gestor Electoral intenta cancelarla nuevamente
...
- ¿Se debe mostrar algún orden específico de candidatos en el tarjetón?
    - El Gestor Electoral decide el criterio de ordenamiento. Por defecto, los candidatos se muestran ordenados por la
...
- **FR-001**: El sistema DEBE permitir al Gestor Electoral inscribir una candidatura (vincular un candidato a una
...
- **FR-002**: El sistema DEBE validar que el candidato esté activo y su partido esté habilitado al inscribir una
  candidatura.
- **FR-003**: El sistema DEBE permitir al Gestor Electoral cancelar una candidatura solo cuando la votación esté en
  estado `ACTIVA`.
...
- **FR-010**: Solo el rol Gestor Electoral DEBE poder inscribir y cancelar candidaturas.
...
- **SC-001**: El Gestor Electoral puede asignar múltiples candidatos a una votación en una sola operación.
