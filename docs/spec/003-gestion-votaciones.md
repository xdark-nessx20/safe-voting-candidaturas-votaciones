...
El Gestor Votaciones transiciona una votación de estado `EN_PROGRESO` a `FINALIZADA`, cerrando la recepción de votos e
iniciando la etapa de auditoría, donde se revisan y pueden anularse votos antes de que los resultados se hagan públicos.

**Why this priority**: Es el cierre oficial de la jornada. Marca el fin de la recepción de votos y da paso al proceso de
auditoría.

**Independent Test**: Puede probarse con una votación `EN_PROGRESO`, cerrándola y verificando que su estado cambia a
`FINALIZADA`, no se aceptan más votos, y se habilita la anulación de votos.
...
El Gestor Votaciones suspende una votación antes de que finalice, marcándola como `CANCELADA`. La cancelación requiere
...
El Gestor Votaciones define y modifica las fechas de inicio y fin de una votación mientras esté en estado `ACTIVA` o
...
El Gestor Votaciones transiciona una votación de estado `FINALIZADA` a `COMPLETADA`, una vez finalizado el proceso de
...
- ¿Qué sucede si se intenta abrir una votación antes de su fecha de inicio?
    - No se permite abrir una votación antes de su `fecha_inicio`. El sistema DEBE rechazar la apertura si la fecha
      actual es anterior a `fecha_inicio`. Sin embargo, el Gestor Votaciones puede modificar `fecha_inicio` y
...
- **FR-005**: El sistema DEBE permitir al Gestor Votaciones establecer y modificar `fecha_inicio` y `fecha_fin` en
...
- **FR-007**: El sistema DEBE permitir al Gestor Votaciones abrir una votación (`ACTIVA` → `EN_PROGRESO`), solo si tiene
  candidatos asignados y la fecha actual es mayor o igual a `fecha_inicio`.
- **FR-008**: El sistema DEBE permitir al Gestor Votaciones cerrar una votación (`EN_PROGRESO` → `FINALIZADA`), dando
  inicio a la etapa de auditoría.
- **FR-009**: El sistema DEBE permitir al Gestor Votaciones cancelar una votación (`ACTIVA` o `EN_PROGRESO` →
...
- **FR-011**: El sistema DEBE permitir al Gestor Votaciones completar una votación (`FINALIZADA` → `COMPLETADA`),
